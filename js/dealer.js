...
async function startBetting(S){if(!['lobby','settled'].includes(S.meta.state))return;const next=S.meta.round+1,updates={'meta/state':'betting','meta/round':next,'meta/turnDeadline':null,reactions:null};const old=next-KEEP_ROUNDS;if(old>=1)updates[`rounds/${old}`]=null;await F.update(F.ref(db,roomPath(S)),updates);}
